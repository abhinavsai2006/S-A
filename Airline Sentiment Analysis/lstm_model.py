# Airline Sentiment Analysis - LSTM Model
# This script trains and tests an LSTM model for airline sentiment classification.
...
class LSTMClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim=64, hidden_dim=128, n_classes=3, n_layers=1, dropout=0.3):
        super(LSTMClassifier, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embedding_dim, padding_idx=0)
        self.lstm = nn.LSTM(embedding_dim, hidden_dim, n_layers, dropout=dropout if n_layers > 1 else 0, batch_first=True)
...
def train_and_evaluate_model():
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01)  # Higher learning rate for simpler model
    ...
    # Save vocabulary
    import json
    with open('lstm_vocab.json', 'w') as f:
        json.dump(vocab, f)

    print("\nModel saved as 'lstm_model.pth'")
    print("Vocabulary saved as 'lstm_vocab.json'")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("Airline Sentiment Analysis - LSTM")
    print("="*50)
