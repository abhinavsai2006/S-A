# Airline Sentiment Analysis - CNN Model
# This script trains and tests a CNN model for airline sentiment classification.
...
class CNNClassifier(nn.Module):
    def __init__(self, vocab_size, embedding_dim=128, n_classes=3, dropout=0.5):
        ...
        self.fc = nn.Linear(300, n_classes)  # 100 * 3 = 300

    def forward(self, input_ids):
        embedded = self.embedding(input_ids).unsqueeze(1)  # Add channel dimension
        ...
        # Max pooling
        pool1 = torch.max_pool1d(conv1_out, conv1_out.size(2)).squeeze(2)
        pool2 = torch.max_pool1d(conv2_out, conv2_out.size(2)).squeeze(2)
        pool3 = torch.max_pool1d(conv3_out, conv3_out.size(2)).squeeze(2)

        # Concatenate
        cat = torch.cat((pool1, pool2, pool3), 1)
...
def train_and_evaluate_model():
    ...
    # Save vocabulary
    import json
    with open('cnn_vocab.json', 'w') as f:
        json.dump(vocab, f)

    print("\nModel saved as 'cnn_model.pth'")
    print("Vocabulary saved as 'cnn_vocab.json'")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*50)
    print("Airline Sentiment Analysis - CNN")
    print("="*50)
